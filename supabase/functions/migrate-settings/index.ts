...
serve(async (req) => {
  ...
  try {
    ...
    const { data: secretsData, error: secretsError } =
      await supabaseClient.functions.invoke("get-secrets", {
    ...
    if (secretsError) {
      ...
      throw new Error(
        `Failed to fetch current secrets: ${secretsError.message}`
      );
    }

    const currentSecrets = secretsData || {};
    logStep("Current secrets fetched", {
      count: Object.keys(currentSecrets).length,
    });
    ...
    if (existingError) {
      logStep("Error fetching existing settings", {
        error: existingError.message,
      });
      throw new Error(
        `Failed to fetch existing settings: ${existingError.message}`
      );
    }
    ...
    logStep("Existing settings checked", {
      count: existingSettings?.length || 0,
    });
    ...
    for (const [secretKey, secretValue] of Object.entries(currentSecrets)) {
      ...
      try {
        ...
        const { data: upsertData, error: upsertError } =
          await supabaseClient.rpc("upsert_setting", {
        ...
        if (upsertError) {
          logStep("Error migrating setting", {
            secretKey,
            settingKey,
          ...
          logStep("Setting migrated successfully", {
            secretKey,
            settingKey,
        ...
        const errorMessage =
          settingError instanceof Error
            ...
            : String(settingError);
        logStep("Error processing setting migration", {
          secretKey,
          settingKey,
          error: errorMessage,
        });
