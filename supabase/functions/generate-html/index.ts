...
serve(async (req) => {
  ...
  console.log(
    `[GENERATE-HTML] v1.3 SERVICE-ROLE-KEY - ${new Date().toISOString()}`
  );

  try {
    // Usar SERVICE_ROLE_KEY para acessar configurações diretamente (como get-public-settings faz)
    console.log(
      "[GENERATE-HTML] Buscando configurações com SERVICE_ROLE_KEY..."
    );
    ...
    if (settingsError) {
      console.error(
        "[GENERATE-HTML] Erro ao buscar configurações:",
        settingsError
      );
    ...
    const companyName = fixUTF8(config.company_name || "MoneyZap Já");
    const companyDescription = fixUTF8(
      config.company_description || "Seu assistente financeiro pessoal"
    );
    const companyLogo = config.logo_url || "/placeholder.svg";

    console.log("[GENERATE-HTML] Valores finais:", {
      companyName,
      companyDescription,
      companyLogo,
    });
    ...
    console.log(
      "[GENERATE-HTML] HTML gerado (início):",
      ...
      JSON.stringify({
        ...
        data: {
          ...
          instructions: {
            ...
            step2:
              "Substitua o conteúdo do arquivo index.html no seu repositório GitHub",
