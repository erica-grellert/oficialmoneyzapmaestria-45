...
serve(async (req) => {
  ...
  try {
    ...
    if (isAnonymousToken(token)) {
      ...
      throw new Error(
        "Token anônimo detectado. É necessário estar autenticado com uma conta válida para criar uma sessão de checkout."
      );
    }
    ...
    const { data: userData1, error: userError1 } =
    ...
    const { data: priceData, error: priceError } =
      await supabaseClient.functions.invoke("get-stripe-prices");

    if (priceError || !priceData?.success) {
      logStep("ERROR: Failed to fetch price IDs", {
        error: priceError?.message || priceData?.error,
      });
      throw new Error(
        `Erro ao buscar configurações de preço: ${
          priceError?.message || priceData?.error || "Unknown error"
        }`
      );
    }
    ...
    if (!priceId) {
      throw new Error(
        `ID de preço não encontrado para o tipo de plano: ${planType}`
      );
    }

    // Verificar se o priceId corresponde a um dos valores esperados
    const validPriceIds = [priceData.prices.monthly, priceData.prices.annual];

    if (!validPriceIds.includes(priceId)) {
      logStep("ERROR: Invalid priceId", {
    ...
    // Buscar chave secreta do Stripe da tabela moneyzap_settings
    ...
    if (stripeKeyError || !stripeKeyData?.value) {
      logStep("ERROR: Stripe secret key not found in database", {
        error: stripeKeyError?.message,
      });
      throw new Error(
        "Chave secreta do Stripe não configurada. Entre em contato com o administrador."
      );
    }
