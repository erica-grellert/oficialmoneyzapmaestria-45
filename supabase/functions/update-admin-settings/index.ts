...
serve(async (req) => {
  ...
  try {
    ...
    if (!category || !updates || typeof updates !== "object") {
      throw new Error(
        "Invalid request: category and updates object are required"
      );
    }
    ...
    for (const [key, value] of Object.entries(updates)) {
      try {
        ...
        if (upsertError) {
          logStep("Error upserting setting", {
            category,
            key,
          ...
          logStep("Setting updated successfully", {
            category,
            key,
        ...
        const errorMessage =
          settingError instanceof Error
            ...
            : String(settingError);
        logStep("Error processing setting", {
          category,
          key,
          error: errorMessage,
        });
