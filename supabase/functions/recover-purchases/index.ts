    ...
    for (const session of sessions.data) {
      try {
        ...
        if (!existingUser.user) {
          ...
          // Criar em moneyzap_users
          await supabase.from("moneyzap_users").insert({
            id: newUser.user!.id,
            email: customerEmail,
          ...
          processedCount++;
          logStep("User recovered successfully", {
            email: customerEmail,
            userId: newUser.user!.id,
          });
        }
      } catch (error) {
        logStep("Error processing session", {
          sessionId: session.id,
          error: error.message,
        });
        errorCount++;
