export async function handleSubscriptionUpdated(
  ...
  try {
    ...
    // Verify the user exists in moneyzap_users table and get the correct ID
    const { data: user, error: userError } = await supabase
      .from("moneyzap_users")
      .select("id")
      .eq("id", userId) // moneyzap_users.id = auth.users.id
      .single();

    if (userError || !user) {
      console.error(
        `User not found in moneyzap_users table for userId: ${userId}`,
        { error: userError }
      );
      return;
    }
    ...
    if (subscription.current_period_start) {
      ...
      console.log(
        `Setting current_period_start: ${subscriptionData.current_period_start}`
      );
    }
    ...
    if (subscription.current_period_end) {
      ...
      console.log(
        `Setting current_period_end: ${subscriptionData.current_period_end}`
      );
    }
    ...
    if (
      ...
      try {
        ...
        if (planConfigData?.value) {
          ...
          console.log(
            `[SUBSCRIPTION-UPDATED] Plan config from DB:`,
            planConfig
          );
          ...
          if (priceId === planConfig.prices?.monthly?.priceId) {
            ...
            console.warn(
              `[SUBSCRIPTION-UPDATED] Unknown price ID: ${priceId}. Using interval fallback.`
            );
        ...
        console.error(
          "[SUBSCRIPTION-UPDATED] Error fetching plan config, using interval fallback:",
          error
        );
        planType = interval === "year" ? "annual" : "monthly";
      }
      ...
      console.log(
        `[SUBSCRIPTION-UPDATED] Final plan type determined: ${planType} for price ${priceId} with interval ${interval}`
      );

      if (planType) {
        subscriptionData.plan_type = planType;
        console.log(
          `[SUBSCRIPTION-UPDATED] Setting plan_type to ${planType} for subscription ${subscription.id}`
        );
      }
    }
