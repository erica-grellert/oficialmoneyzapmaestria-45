export async function handleCheckoutSessionCompleted(
  ...
  // Get the moneyzap_users ID from the auth user ID
  const { data: user, error: userError } = await supabase
    .from("moneyzap_users")
    .select("id")
    .eq("id", authUserId) // moneyzap_users.id = auth.users.id
    .single();
  ...
  if (userError || !user) {
    ...
    throw new Error(
      `User not found in moneyzap_users table: ${
  ...
  try {
    ...
    console.error(
      "Error fetching plan config, using interval fallback:",
      error
    );
  ...
  console.log(
    `Processing subscription for price ID: ${priceId}, plan type: ${planType}`
  );
  ...
  await supabase.from("moneyzap_subscriptions").upsert({
    user_id: userId,
  ...
  console.log(
    `Subscription created/updated with plan ${planType} and status ${subscriptionStatus}`
  );
}
