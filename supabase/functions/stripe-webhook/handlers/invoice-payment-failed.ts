export async function handleInvoicePaymentFailed(
  ...
  console.log(
    `Updating subscription ${subscriptionId} to status: ${subscription.status} after payment failure`
  );
  ...
  console.log(
    `Subscription ${subscriptionId} updated with failed payment status: ${subscription.status}`
  );
}
