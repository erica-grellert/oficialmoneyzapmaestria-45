  ...
  try {
    ...
    for (const subscription of subscriptions.data) {
      try {
        ...
        if (!customer.email) {
          logStep("Skipping subscription without customer email", {
          ...
          if (subscription.items.data.length > 0) {
            ...
            try {
              const response = await fetch(
                `${Deno.env.get("SUPABASE_URL")}/functions/v1/get-plan-config`,
                {
                  method: "POST",
                  headers: {
              ...
              console.log(
                "Error fetching plan config, using fallback logic",
                error
              );
              // Fallback para detecção por interval
              if (
