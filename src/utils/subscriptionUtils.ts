...
export async function getPlanConfig() {
  ...
  try {
    ...
    if (priceError || !priceData?.success) {
      throw new Error(
        priceError?.message || priceData?.error || "Failed to fetch price IDs"
      );
    }

    // Buscar configurações públicas de pricing
    const { data: adminData, error: adminError } =
      await supabase.functions.invoke("get-public-settings", {
        ...
        prices: {
          ...
          annual: {
            ...
            displayOriginalPrice:
              settings.plan_price_monthly_equivalent || "R$ -",
            displaySavings: `Economize ${
              settings.plan_discount_percentage || "-"
            }%`,
          },
        },
        contact: {
 ...
 */
export async function getPlanTypeFromPriceId(
  priceId: string
  ...
  console.error(
    `PriceId inválido: ${priceId}. Não corresponde a nenhum plano conhecido.`
  );
  return null;
}
 ...
 */
export async function getPriceIdFromPlanType(
  ...
  console.error(
    `PlanType inválido: ${planType}. Deve ser 'monthly' ou 'annual'.`
  );
  return null;
}
 ...
 */
export function getPlanTypeFromPriceIdSync(
  priceId: string
): "monthly" | "annual" | null {
 ...
 * Usa o cache se disponível, caso contrário retorna null
 */
export function getPriceIdFromPlanTypeSync(
  planType: "monthly" | "annual"
): string | null {
  if (!planConfigCache) return null;
