  ...
  useEffect(() => {
    if (success === "true") {
      toast({
        ...
        description: "Sua assinatura foi ativada. Bem-vindo ao MoneyZap!",
      });
      navigate("/dashboard");
      ...
      const { data, error } = await supabase.functions.invoke(
        "create-checkout-session",
        {
          body: {
            ...
            }/payment-success?email=${encodeURIComponent(user.email || "")}`,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {plans.map((planItem) => (
            <Card
              key={planItem.name}
              className={`relative ${
                planItem.popular ? "border-primary shadow-xl" : ""
              }`}
            >
              {planItem.popular && (
              ...
              <CardContent>
                ...
                <Button
                  ...
                  size="lg"
                  onClick={() =>
                    handleCheckout(planItem.priceId, planItem.planType)
                  }
                  disabled={isLoading}
                >
                  {isLoading ? (
