...
function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case "SET_CUSTOM_DATE_RANGE":
      return {
        ...state,
        customStartDate: action.payload.start,
        customEndDate: action.payload.end,
      };
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const loadUserData = async (user: any) => {
    ...
    try {
      ...
      const [transactionsRes, categoriesRes, goalsRes] = await Promise.all([
        ...
        supabase.from("moneyzap_categories").select("*").eq("user_id", user.id),
        supabase.from("moneyzap_goals").select("*").eq("user_id", user.id),
      ]);
      ...
      const transactions = (transactionsRes.data || []).map(
  ...
  const getTransactions = useCallback(async (): Promise<Transaction[]> => {
    try {
      ...
      console.log(
        "AppContext: Transactions fetched successfully:",
        transactions.length
      );
      dispatch({ type: "SET_TRANSACTIONS", payload: transactions });
      return transactions;
  ...
  const updateUserProfile = async (data: any): Promise<void> => {
    try {
      ...
      const { updateUserProfile: updateUserProfileService } = await import(
  ...
  const addTransaction = async (
    ...
    try {
      ...
      console.log(
        "AppContext: Transaction added successfully:",
...
// Add a safe context hook that can handle both contexts
export const useSafeAppContext = () => {
  try {
    return useAppContext();
  } catch (error) {
    console.warn("AppContext not available, returning fallback:", error);
