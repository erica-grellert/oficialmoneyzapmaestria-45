...
import { TrendingUp, TrendingDown, Mic, Camera, MessageSquare, ArrowRight } from 'lucide-react';
...
const MZHero: React.FC<MZHeroProps> = ({
  ...
  const profitGoalProgress = monthlyProfitGoal > 0 
    ? Math.min(Math.max((monthlyProfitAchieved || netValue) / monthlyProfitGoal, 0), 1) * 100
    : 0;

  const shouldShowProfitGoal = monthlyProfitGoal > 0;

  return (
    <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-[var(--mz-emerald-600)] to-[var(--mz-mint-500)] p-6 lg:p-8 shadow-md">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between mb-6">
          <div className="mb-6 lg:mb-0 flex-1">
            <div className="flex items-center justify-between mb-2">
              ...
              <Link 
                to={`/reports?month=${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, '0')}`}
            ...
            <div className="flex items-baseline gap-3 mb-4">
              ...
              <div className={`flex items-center gap-1 ${changeIsPositive ? 'text-green-200' : 'text-red-200'}`}>
                {changeIsPositive ? (
            ...
            {shouldShowProfitGoal && (
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 mb-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-white/90">Meta de lucro do mês</span>
                  <span className="text-sm font-bold text-white">{profitGoalProgress.toFixed(0)}% atingido</span>
                </div>
                ...
                <div className="flex justify-between text-xs text-white/75 mt-2">
                  <span>{formatCurrency(monthlyProfitAchieved || netValue)}</span>
