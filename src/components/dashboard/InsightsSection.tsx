...
import { TrendingUp, AlertCircle, Target, DollarSign, ArrowRight } from 'lucide-react';
...
const InsightsSection: React.FC<InsightsSectionProps> = ({ insights, onInsightAction }) => {
  ...
  const handleInsightClick = (insight: Insight) => {
    if (insight.action.type === 'navigate' && insight.action.route) {
      if (insight.action.filter) {
        ...
        const url = queryString ? `${insight.action.route}?${queryString}` : insight.action.route;
        navigate(url);
      } else {
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-xl font-semibold text-gray-900">Insights Financeiros</h3>
      
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {insights.map((insight) => (
          <div
            key={insight.id}
            className={`bg-white rounded-xl p-4 border-2 transition-all duration-200 cursor-pointer shadow-md hover:shadow-lg hover:scale-[1.01] hover:-translate-y-1 ${getInsightStyles(insight.type)} focus:outline-none focus:ring-2 focus:ring-emerald-600/50`}
        ...
        {insights.length === 0 && (
          <div className="col-span-full text-center py-12 text-gray-500">
            ...
            <p className="text-lg font-medium mb-1">Nenhum insight disponível no momento</p>
            <p className="text-sm">Continue usando o MoneyZap para receber dicas personalizadas</p>
          </div>
        )}
      </div>
    </div>
